...
mod patient_tests {
    ...
    async fn test_patient_db_connection() {
        ...
        assert!(
            result.is_ok(),
            "Should be able to run a simple patient-related query"
        );
        ...
        match tables_check {
            Ok(result) => {
                ...
                println!(
                    "Note: This is just information about whether the patient table exists in the database"
                );
                println!("The test will pass regardless of whether the table exists or not");
            }
            Err(e) => {
                println!(
                    "Patient table check failed, but this is not a test failure: {}",
                    e
                );
                println!(
                    "This can happen if the database doesn't support the query or if tables don't exist yet"
                );
            }
        }
    }
