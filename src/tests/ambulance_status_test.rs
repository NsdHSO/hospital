...
#[actix_rt::test]
    async fn test_ambulance_statuses() {
        ...
        let statuses = response["message"].as_array().expect("Expected 'message' field to be an array");
        
        // Print the entire response for debugging
        println!("Full response: {}", serde_json::to_string_pretty(&response).unwrap());
        
        // Assert the number of statuses
        assert_eq!(statuses.len(), AmbulanceStatusEnum::iter().count(), "Expected {} statuses, got {}", AmbulanceStatusEnum::iter().count(), statuses.len());
        
        // Check if each status has 'value' and 'label' fields
        for (i, status) in statuses.iter().enumerate() {
            assert!(status.get("value").is_some(), "Status at index {} is missing 'value' field", i);
            assert!(status.get("label").is_some(), "Status at index {} is missing 'label' field", i);
            println!("Status {}: value='{}', label='{}'", 
                i, 
        ...
        let expected_statuses = [
            ...
            "EN_ROUTE_TO_SCENE", "AT_SCENE", "TRANSPORTING_PATIENT", "EN_ROUTE_TO_HOSPITAL",
            "AT_HOSPITAL", "RETURNING_TO_BASE", "UNAVAILABLE", "OUT_OF_SERVICE",
            "ON_BREAK", "FUELING", "CLEANING", "AWAITING_DISPATCH",
            "PREPARING_FOR_MISSION", "UNDER_REPAIR"
        ];
        
        let status_values: Vec<&str> = statuses.iter()
        ...
        assert_eq!(status_values.len(), 18, "Expected 18 statuses, got {}", status_values.len());
        
        // Check each expected status is present
        for expected in expected_statuses.iter() {
            let found = status_values.iter().any(|s| *s == *expected);
            assert!(found, "Expected status '{}' not found in response", expected);
        }
    }
